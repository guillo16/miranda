class SubscriptionsController < ApplicationController
  ...
  def cancel
    @user = current_user
    return redirect_to root_path, alert: "You are not subscribed" unless @user.plan_id
    sub = Stripe::Subscription.retrieve(@user.plan_id)
    sub.delete
  ...
  def subscription_checkout
    ...
    flash[:notice] = "Your free trial is now active.  Your first charge will be in 7 days."
    current_user.update(subscribed: true)
    current_user.plan_id = stripe_subscription.id
    current_user.save
    if current_user.save
      redirect_to videos_path
    end
